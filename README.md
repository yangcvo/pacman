...
One
```
$ git clone https://github.com/yangcvo/pacman.git themes/pacman
```
...
## Attribute function
**Menu menu**
Default is not enabled /tags and /categories page if need to enable please blog directories in the source folder are established tags and categories for each folder in the folder containing a index.md file. Content as:
```
One
Two
Three
Tags layout: (or categories)
Tags title: (or categories)
-
```
Because the template has been built in these two pages, they will be parsed correctly.
Control widgets
Provides 7 kinds of gadgets. Including tags, tag clouds, classification, archiving, RSS, links, micro-blog show.
